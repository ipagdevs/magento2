...
- Cancela de fato o pedido quando a API devolve cartão negado ou cancelado (status 3 e 7), chamando o `cancel()` do Magento. Antes apenas o status do pedido era ajustado para o valor configurado em `payment/ipagbase/order_status`, sem passar pela rotina de cancelamento do Magento — que é a que libera a reserva de estoque dos itens.
