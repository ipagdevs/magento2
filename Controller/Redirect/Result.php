...
class Result extends \Magento\Framework\App\Action\Action implements CsrfAwareActionInterface
{
     ...
     *
     * Atenção: essa saída é silenciosa para o cliente. O addError abaixo acompanha o que os dois
     * catch de execute() já faziam, mas foi verificado que a mensagem não renderiza — nem na home,
     * nem na seção messages do customer-data, nem numa página não-cacheável como checkout/cart.
     * Quem registra o motivo é o log. Só é alcançável por token que aponta para um pedido ausente
     * do Magento (pedido apagado, ou token de outro ambiente).
     *
     * @param \Magento\Sales\Model\Order|null $order
